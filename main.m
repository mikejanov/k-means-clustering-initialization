...
% --------------------
% 1) Randomized Means
% 2) Cheng's Two Pass Linear
% 3) Yuan's Density
% Incomplete: 4) Kaufman Approach
...
num_diff_inits = 3; % Three different initialization types
...
time_density = zeros(num_diff_data_sets,1);

num_iterations_lakm = zeros(num_diff_data_sets,1);
num_iterations_density = zeros(num_diff_data_sets,1);
...
[gauss2_mus(:,:,3),...
    gauss2_classified(:,size(gauss2,2),3),...
    time_density(1),...
    num_iterations_density(1)] =...
cluster_density_init(gauss2_num_k,...
                     gauss2(:,1:gauss2_num_dimms));
...
[iris_mus(:,:,3),...
    iris_classified(:,size(iris,2),3),...
    time_density(2),...
    num_iterations_density(2)] =...
cluster_density_init(iris_num_k,...
                     iris(:,1:iris_num_dimms));
...
[wine_mus(:,:,3),...
    wine_classified(:,size(wine,2),3),...
    time_density(3),...
    num_iterations_density(3)] =...
cluster_density_init(wine_num_k,...
                     wine(:,1:wine_num_dimms));
...
[glass_mus(:,:,3),...
    glass_classified(:,size(glass,2),3),...
    time_density(4),...
    num_iterations_density(4)] =...
cluster_density_init(glass_num_k,...
                     glass(:,1:glass_num_dimms));
...
subplot(2,2,1)
plotClasses(gauss2_classified(:,:,3), gauss2_mus(:,:,3), gauss2_num_k, gauss2_x, gauss2_y, 0);
add_plot_info('gauss2', gauss2_x, gauss2_y);
title('\{Init: Density\} 2-Class Multivariate Gaussian Data')

subplot(2,2,2)
plotClasses(iris_classified(:,:,3), iris_mus(:,:,3), iris_num_k, iris_x, iris_y, 0);
add_plot_info('iris', iris_x, iris_y);
title('\{Init: Density\} Iris Plants Data')

subplot(2,2,3)
plotClasses(wine_classified(:,:,3), wine_mus(:,:,3), wine_num_k, wine_x, wine_y, 0);
add_plot_info('wine', wine_x, wine_y);
title('\{Init: Density\} Wine Recognition Data')

subplot(2,2,4)
plotClasses(glass_classified(:,:,3), glass_mus(:,:,3), glass_num_k, glass_x, glass_y, 0);
add_plot_info('glass', glass_x, glass_y);
title('\{Init:Density\} Glass Identification Data')
...
% disp(accuracy)
% 
% disp(time_randomized)
% disp(time_lakm)
% disp(time_density)
% 
% disp(num_iterations_lakm)
% disp(num_iterations_density)
